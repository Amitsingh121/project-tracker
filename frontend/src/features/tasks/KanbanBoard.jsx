...
import { useTasks, useUpdateTask } from './hooks.js';
...
function DraggableTaskCard({ task, project, projectId, user, canDrag }) {
...
function DroppableColumn({ status, label, dot, children, count }) {
...
export default function KanbanBoard({ projectId, project }) {
  ...
  // NOTE: tried SortableContext here but it was overkill for column-only DnD
  ...
  const onDragEnd = async ({ active, over }) => {
    ...
    qc.setQueryData(['tasks', projectId], (prev) =>
    ...
    try {
      ...
      toast.error(err.response?.data?.error?.message || 'Could not move task');
    }
  };
